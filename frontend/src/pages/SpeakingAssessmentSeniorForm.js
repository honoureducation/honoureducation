      ...
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <nav className="flex items-center gap-2 text-sm text-slate-500 mb-4 font-medium">
            ...
            <span className="text-slate-300">/</span>
            <span className="text-slate-900 font-semibold">Senior</span>
          </nav>
          
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Speaking Assessment</h1>
              <p className="text-slate-500 text-sm mt-1 font-medium uppercase tracking-wider">Year 10-13 / Grade 9-12</p>
            </div>
            {cefrLevel && (
              <div className="flex flex-col items-end">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Current CEFR</span>
                <span className="px-4 py-1.5 bg-orange-50 text-orange-700 border border-orange-200 rounded-full font-bold text-lg">
                  Level {cefrLevel}
                </span>
              </div>
            )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all text-slate-900"
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  value={formData.term}
                  onChange={handleInputChange}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all text-slate-900 appearance-none"
                  required
                >
                      ...
                      <td className="px-6 py-5 text-center">
                        <label className="relative flex items-center justify-center cursor-pointer group">
                          <input
                            ...
                            className="sr-only"
                          />
                          <div className={`w-6 h-6 rounded-full border-2 transition-all flex items-center justify-center ${scores[index] === 1 ? 'border-orange-500 bg-orange-50' : 'border-slate-200 group-hover:border-slate-300'}`}>
                            {scores[index] === 1 && <div className="w-2.5 h-2.5 rounded-full bg-orange-500" />}
                          </div>
                        </label>
                      </td>
                      <td className="px-6 py-5 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={item.level} className={`p-3 rounded-xl border text-center transition-all ${cefrLevel === item.level ? 'bg-orange-50 border-orange-200' : 'bg-slate-50/50 border-slate-100'}`}>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{item.range} Pts</p>
                    <p className={`text-sm font-bold ${cefrLevel === item.level ? 'text-orange-600' : 'text-slate-700'}`}>{item.level}</p>
            ...
            <div className="bg-slate-900 rounded-2xl shadow-xl p-6 flex flex-col justify-between text-white relative overflow-hidden group">
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-white/10">
                <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center font-black text-orange-400">
                  {cefrLevel || '?'}
                </div>
                <div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            ...
            <textarea
              ...
              className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all text-slate-900 h-28 resize-none text-sm"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-4">
            <button
              type="submit"
              disabled={loading}
              className="flex-1 px-8 py-4 bg-gradient-to-r from-orange-500 to-red-600 text-white font-bold rounded-2xl hover:from-orange-600 hover:to-red-700 active:scale-[0.98] transition-all disabled:opacity-50 disabled:pointer-events-none shadow-xl shadow-orange-600/20 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
